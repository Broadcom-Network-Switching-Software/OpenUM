...
/*! CMICx led firmware interface. */
#include "cmicx_customer_led.h"

/*! Shared information between LED example FW and UM. */
...
customer_led_handler(soc_led_custom_handler_ctrl_t *ctrl, uint32 cnt)
{
    ...
    if (ctrl->led_control_data[LED_OPTION] == 1) {
        led1_offset = 0;
        led2_offset = 2;
    } else {
        led1_offset = 2;
    ...
    for (led_uc_port = 0; led_uc_port < 384; led_uc_port ++) {
        ...
        if (ctrl->led_control_data[led_uc_port] & LED_SW_LINK_UP) {
            ...
            pattern &= ~(0x3 << led1_offset);
            pattern |= (LED_GREEN << led1_offset);
