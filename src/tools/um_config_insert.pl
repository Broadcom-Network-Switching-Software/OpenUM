	  ...
	  if (defined($variable_hash->{"board_name"})) {
	      chomp $image_info_hash->{"CFG_BOARDNAME"};
	  	  if ($image_info_hash->{"CFG_BOARDNAME"} ne $variable_hash->{"board_name"}) {
	  	  	  print "[Warning] \"board_name\" of the config file is different with the image.\n";
                          print "[Warning] You may insert a wrong config into the image. \n";
                          print "Do you want to insert this config into the image anyway? (N/y):"; 
                          my $c = getc(STDIN);
                          if (uc($c) eq 'Y') {
                              print "Confirmed, proceed to insert the config into the image\n";  
                          } else {
                              error "program exit because of board_name checking fail\n";
                          }
	  	  }
	  }
