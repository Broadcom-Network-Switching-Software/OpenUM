...
extern soc_switch_t soc_switch_bcm5357x;
...
board_lport_to_uport(uint8 unit, uint8 lport, uint16 *uport)
{
   ...
   switch (gh2_sw_info.devid) {
    ...
    if (uport == NULL || lport < BCM5357X_LPORT_MIN || lport > BCM5357X_LPORT_MAX || unit > 0) {
        return SYS_ERR_PARAMETER;
    }
...
board_get_port_link_status(uint16 uport, BOOL *link)
{
    ...
    return (*soc_switch_bcm5357x.link_status)(unit, lport, link);
}
            ...
            if (enable == TRUE) {
                ...
                SAL_UPORT_ITER(uport) {
                    ...
                    if (PBMP_MEMBER(lag_group[i-1].lpbmp, lport)) {
                        SOC_IF_ERROR_RETURN(
                            (*soc_switch_bcm5357x.link_status)(0, lport, &link));
                        if (!link) {
                ...
                SOC_IF_ERROR_RETURN(
                    bcm5357x_lag_group_set(0, (i - 1), hw_lpbmp));
            } else {
                /* remove lag setting from HW */ 
                PBMP_CLEAR(hw_lpbmp);
                SOC_IF_ERROR_RETURN(bcm5357x_lag_group_set(0, (i - 1), hw_lpbmp));
            }
        }
        lag_enable = enable;
    }
...
board_lag_group_set(uint8 lagid, uint8 enable, uint8 *uplist)
{
    ...
    /* HW setting */
    SOC_IF_ERROR_RETURN(bcm5357x_lag_group_set(0, (lagid - 1), hw_lpbmp));
    ...
    if (lag_enable == TRUE) {
        ...
        for (i = 1 ; i <= BOARD_MAX_NUM_OF_LAG ; i++) {
            if ((lag_group[i-1].enable == TRUE) && 
                ...
                /* add lag setting to HW */
                bcm5357x_lag_group_get(0, (i - 1), &hw_lpbmp);
                if (link) {
...
board_untagged_priority_set(uint16 uport, uint8 priority)
{
    ...
    WRITE_FP_POLICY_TABLEm(unit,QOS_BASE_IDX + (lport - BCM5357X_LPORT_MIN), fp_policy_table);
    ...
    FP_TCAMm_VALIDf_SET(fp_tcam, 3);
    WRITE_FP_TCAMm(unit,QOS_BASE_IDX + (lport - BCM5357X_LPORT_MIN), fp_tcam);

    
    FP_GLOBAL_MASK_TCAMm_CLR(fp_global_mask_tcam);
    FP_GLOBAL_MASK_TCAMm_VALIDf_SET(fp_global_mask_tcam, 1);
    WRITE_FP_GLOBAL_MASK_TCAMm(unit,QOS_BASE_IDX + (lport - BCM5357X_LPORT_MIN),fp_global_mask_tcam);
...
board_untagged_priority_get(uint16 uport, uint8 *priority)
{
    ...
    rv = READ_FP_POLICY_TABLEm(unit, QOS_BASE_IDX + (lport - BCM5357X_LPORT_MIN), fp_policy_table);
...
board_port_rate_ingress_set(uint16 uport, uint32 bits_sec)
{
    ...
    if (0 == bits_sec) {
        /* no limit */
        READ_FP_TCAMm(unit, RATE_IGR_IDX + lport - BCM5357X_LPORT_MIN, fp_tcam);
        FP_TCAMm_VALIDf_SET(fp_tcam, 0);
        WRITE_FP_TCAMm(unit, RATE_IGR_IDX + lport - BCM5357X_LPORT_MIN, fp_tcam);
        return SYS_OK;
    } else {
        READ_FP_TCAMm(unit, RATE_IGR_IDX + lport - BCM5357X_LPORT_MIN, fp_tcam);
        FP_TCAMm_VALIDf_SET(fp_tcam, 3);
    ...
    READ_FP_POLICY_TABLEm(unit, RATE_IGR_IDX + lport - BCM5357X_LPORT_MIN, fp_policy_table);
...
board_port_rate_ingress_get(uint16 uport, uint32 *bits_sec)
{
    ...
    *bits_sec = 0x0;
    ...
    SOC_IF_ERROR_RETURN(READ_FP_POLICY_TABLEm(unit, RATE_IGR_IDX + lport - BCM5357X_LPORT_MIN, fp_policy_table));
...
board_port_stat_get(uint16 uport, port_stat_t *stat)
{
    ...
    HOLD_COS_PORT_SELECTr_t hold_cos_port_select;
    ...
    HOLD_COS_PORT_SELECTr_CLR(hold_cos_port_select);
    HOLD_COS_PORT_SELECTr_PORT_NUMf_SET(hold_cos_port_select, lport);
    WRITE_HOLD_COS_PORT_SELECTr(unit, hold_cos_port_select);

    stat->EgressDropPkts_lo = 0x0;
    for (i = 0 ; i < COS_QUEUE_NUM ; i++) {
        rv |= READ_HOLD_COSr(unit ,i, hold_cos);
        stat->EgressDropPkts_lo += HOLD_COSr_COUNTf_GET(hold_cos);
    }
...
board_port_stat_clear(uint16 uport)
{
    ...
    for (i = 0 ; i < COS_QUEUE_NUM ; i++) {
        ...
        rv |= WRITE_HOLD_COSr(unit ,i, hold_cos);
    }
...
board_mcast_addr_add(uint8 *mac_addr, uint16 vlan_id, uint8 *uplist)
{
    ...
    if (mlist == NULL) {
       ...
       mcast->next = mlist;
       mcast->index = 0;
       for (i = 0; i < gh2_sw_info.l2_mc_size; i++) {
           for (j=0; j<32; j++) {
                if ((mcindex[i] & (1<<j)) == 0) {
    ...
    rv = bcm5357x_l2_op(0, &l2x, SC_OP_L2_INS_CMD, NULL);
...
board_mcast_addr_remove(uint8 *mac_addr, uint16 vlan_id)
{
    ...
    l2x.port = l2_index;
    sal_memcpy(l2x.mac_addr, mac_addr, 6);
    rv = bcm5357x_l2_op(0, &l2x, SC_OP_L2_DEL_CMD, NULL);
...
board_block_unknown_mcast_get(uint8 *enable)
{
    ...
    *enable = 0;
    rv = READ_UNKNOWN_MCAST_BLOCK_MASK_HI_64r(0, BCM5357X_LPORT_MIN, unknow_mcast_block_mask_hi_64);
...
{
    bcm5357x_loop_detect_enable(enable);
}
...
{
    return bcm5357x_loop_detect_status_get();
}
...
board_set_port_phy_eee_tx_wake_time(uint16 uport, uint8 type, uint16 wake_t)
{
    ...
    r = bcm5357x_port_eee_tx_wake_time_set(unit, lport, type, wake_t);
...
board_port_loopback_enable_set(uint16 uport, int loopback_mode)
{
    ...
    bcm5357x_loopback_enable(unit, lport, loopback_mode);
...
{
    return bcm5357x_mdns_enable_set(0, enable);
}
...
{
    return bcm5357x_mdns_enable_get(0, enable);
}
...
board_snaketest_trap_to_cpu(uint16 vlanid, uint8 *uplist) {
    ...
    return bcm5357x_qvlan_port_set(0, vlanid, lpbmp, tag_lpbmp);
}
...
board_reset_button_get(void)
{
     ...
     * PAD_GPIO 4 ~ 15 : from IPROC
     */
    val = (READCSR(R_CCG_GP_DATA_IN) & 0xfff0) | (READCSR(CMIC_GP_DATA_IN) & 0x000f);
...
board_l2_addr_add(board_l2_addr_t *l2addr)
{
    ...
    rv = bcm5357x_l2_op(0, &l2x, SC_OP_L2_LOOKUP_CMD, &index);
    if (rv == SYS_OK) {
...
board_l2_addr_get_first(board_l2_addr_t *l2addr, uint16 *index)
{
    ...
    for (i=0 ; i< gh2_sw_info.l2_entry_size ; i++) {
    ...
    if (i == gh2_sw_info.l2_entry_size) {
        return SYS_ERR_NOT_FOUND;
    }
...
board_l2_addr_get_next(board_l2_addr_t *l2addr, uint16 *index)
{
    ...
    for (i=(l2_addr_index+1) ; i < gh2_sw_info.l2_entry_size ; i++) {
        rv |= READ_L2_ENTRYm(unit, i, l2_entry);
        if (L2_ENTRYm_VALIDf_GET(l2_entry)) {
            L2_ENTRYm_L2_MAC_ADDRf_GET(l2_entry, mac_addr);
            l2addr->mac[5] = mac_addr[0] & 0xFF;
...
board_l2_addr_get_last(board_l2_addr_t *l2addr, uint16 *index)
{
    ...
    for (i=(gh2_sw_info.l2_entry_size - 1) ; i>=0 ; i--) {
        rv |= READ_L2_ENTRYm(unit, i, l2_entry);
        if (L2_ENTRYm_VALIDf_GET(l2_entry)) {
            L2_ENTRYm_L2_MAC_ADDRf_GET(l2_entry, mac_addr);
            l2addr->mac[5] = mac_addr[0] & 0xFF;
...
* <b>Description:</b> Get the port status for a given port.
* @param uport - Port number.
* @param mode - (OUT) BOARD_PHY_LED_NORMAL , BORAD_PHY_LED_LOOP_FOUND
* @return SYS_OK
* \n      Operation completed successfully.
*/
...
board_phy_led_mode_set(uint8 lport, int mode) {
    ...
    switch (mode) {
        case BOARD_PHY_LED_LOOP_FOUND:
             bcm5357x_ledup_mode_set(0, lport, 0, LED_MODE_BLINK);
             bcm5357x_ledup_mode_set(0, lport, 1, LED_MODE_BLINK);                    
        break;
        case BOARD_PHY_LED_NORMAL:
        default:
            sal_config_uint8_get(SAL_CONFIG_LED_OPTION, &led_option);
            if (led_option == 1) {
                bcm5357x_ledup_mode_set(0, lport, 0, LED_MODE_TXRX);
                bcm5357x_ledup_mode_set(0, lport, 1, LED_MODE_LINK);                    
            } else {
                bcm5357x_ledup_mode_set(0, lport, 0, LED_MODE_LINK);
                bcm5357x_ledup_mode_set(0, lport, 1, LED_MODE_TXRX);                    
            }
        break;
