...
board_mcast_addr_add(uint8 *mac_addr, uint16 vlan_id, uint8 *uplist)
{
    ...
    rv = bcm5346x_l2_op(0, &l2x, SC_OP_L2_INS_CMD, NULL);
...
board_mcast_addr_remove(uint8 *mac_addr, uint16 vlan_id)
{
    ...
    l2x.port = l2_index;
    sal_memcpy(l2x.mac_addr, mac_addr, 6);
    rv = bcm5346x_l2_op(0, &l2x, SC_OP_L2_DEL_CMD, NULL);
...
board_l2_addr_add(board_l2_addr_t *l2addr)
{
    ...
    rv = bcm5346x_l2_op(0, &l2x, SC_OP_L2_LOOKUP_CMD, &index);
    if (rv == SYS_OK) {
...
/*
 * L2_BULK_CONTROL REGISTER : 
 * COMPLETE [Bit 4]
 * START [Bit 3]
 * ACtion [Bit 2 : 1]
 *    0x0 = NO_ACTION - No action
 *    0x1 = DEL_ENTRY - Delete entry
 *    0x2 = REPLACE - Replace entry using data from L2_BULK_REPLACE_EN and L2_BULK_REPLACE_DATA
 *    0x3 = AGE - Perform age action on entry.
 */

/*
 * L2_ENTRY/L2_BULK_MATCH_DATA/L2_BULK_MATCH_MASK TABLE : 
 * VLAN_ID [Bit 14 : 3]
 * MAC_ADDR [Bit 62 : 15]
 * PORT_NUM [Bit 71 : 65]
 * TGID [Bit 69 : 65]
...
board_l2_addr_delete_by_port(uint16 uport)
{
    ...
    uint8 unit = 0, lport;
    int i;
    uint32 entry[4] = {0, 0, 0, 0};

    board_uport_to_lport(uport, &unit, &lport);

    entry[2] = (lport & 0x7F) << 1;
    rv = bcm5346x_mem_set(0, M_L2_BULK_MATCH_DATA, entry, 4);

    entry[2] = 0x7F << 1;
    rv |= bcm5346x_mem_set(0, M_L2_BULK_MATCH_MASK, entry, 4);

    val = (0x1 << 1) | (0x1 << 3);
    bcm5346x_reg_set(unit, R_L2_BULK_CONTROL, val);

    for (i = 0; i < L2_AGE_TIMEOUT_COUNT; i++) {
        sal_usleep(100000);
        bcm5346x_reg_get(unit, R_L2_BULK_CONTROL, &val);
        if (val & (0x1 << 4)) {
            break;
        }
    }
...
board_l2_addr_delete_by_trunk(uint8 lagid)
{
    ...
    entry[2] = ((lagid & 0x1F) << 1) | (0x1 << 16);
    rv = bcm5346x_mem_set(0, M_L2_BULK_MATCH_DATA, entry, 4);

    entry[2] = (0x1F << 1) | (0x1 << 16);
    rv |= bcm5346x_mem_set(0, M_L2_BULK_MATCH_MASK, entry, 4);
...
board_l2_addr_get_first(board_l2_addr_t *l2addr, uint16 *index)
{
    ...
    *index = 0x0;
    ...
    for (i=0 ; i<L2_ENTRY_SIZE ; i++) {    
        rv |= bcm5346x_mem_get(0, M_L2_ENTRY(i), entry, 4);
        if (entry[3] & 0x40) {
