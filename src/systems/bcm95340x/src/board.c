...
board_mcast_addr_add(uint8 *mac_addr, uint16 vlan_id, uint8 *uplist)
{
    ...
    rv = bcm5340x_l2_op(0, &l2x, SC_OP_L2_INS_CMD, NULL);
    
    /* after get mcindex from l2_entry */
    bcm5340x_mem_get(0, M_L2MC(mcast->index), entry, 2);
    entry[0] &= 0x000000ff;
    entry[0] |= ((PBMP_ENTRY(lpbmp, 0) & 0xffffff)<< 8);
    entry[1] &= 0xffffff80;
    entry[1] |= 0x40 | ((PBMP_ENTRY(lpbmp, 0) & 0xff000000) >> 24);
...
board_mcast_addr_remove(uint8 *mac_addr, uint16 vlan_id)
{
    ...
    l2x.port = l2_index;
    sal_memcpy(l2x.mac_addr, mac_addr, 6);
    rv = bcm5340x_l2_op(0, &l2x, SC_OP_L2_DEL_CMD, NULL);
...
board_l2_addr_add(board_l2_addr_t *l2addr)
{
    ...
    rv = bcm5340x_l2_op(0, &l2x, SC_OP_L2_LOOKUP_CMD, &index);
    if (rv == SYS_OK) {
...
board_l2_addr_delete_by_port(uint16 uport)
{
    ...
    val = (lport & 0x3F);
    val |= (0x1 << 28);
    bcm5340x_reg_set(unit, R_PER_PORT_AGE_CONTROL, val);

    for (i = 0; i < L2_AGE_TIMEOUT_COUNT; i++) {
        sal_usleep(100000);
        bcm5340x_reg_get(unit, R_PER_PORT_AGE_CONTROL, &val);
        if (val & (0x1 << 29)) {
            break;
        }
    }
...
board_l2_addr_get_first(board_l2_addr_t *l2addr, uint16 *index)
{
    ...
    *index = 0x0;
    ...
    for (i=0 ; i<L2_ENTRY_SIZE ; i++) {    
        rv |= bcm5340x_mem_get(0, M_L2_ENTRY(i), entry, 4);
        if (entry[3] & 0x10) {
            ...
            lport = ((entry[2] & 0x1F) << 1) | ((entry[1] >> 31) & 0x1);
            if (entry[2] & 0x1000) {
            ...
            if (l2addr->mac[0] & 0x1) {
                /* multicast */
                l2addr->mcidx = ((entry[2] & 0x1FF) << 1) | ((entry[1] >> 31) & 0x1);
            } else {
                /* unicast */
                if (l2addr->is_trunk) {
                    l2addr->tgid = lport + 1;
