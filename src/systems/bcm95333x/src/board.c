...
board_mcast_addr_add(uint8 *mac_addr, uint16 vlan_id, uint8 *uplist)
{
    ...
    rv = bcm5333x_l2_op(0, &l2x, SC_OP_L2_INS_CMD, NULL);
...
board_mcast_addr_remove(uint8 *mac_addr, uint16 vlan_id)
{
    ...
    l2x.port = l2_index;
    sal_memcpy(l2x.mac_addr, mac_addr, 6);
    rv = bcm5333x_l2_op(0, &l2x, SC_OP_L2_DEL_CMD, NULL);
...
board_l2_addr_add(board_l2_addr_t *l2addr)
{
    ...
    rv = bcm5333x_l2_op(0, &l2x, SC_OP_L2_LOOKUP_CMD, &index);
    if (rv == SYS_OK) {
...
board_l2_addr_delete_by_port(uint16 uport)
{
    ...
    bcm5333x_reg_get(unit, R_L2_AGE_TIMER, &val);
    val &= ~(0x1 << 20);
    bcm5333x_reg_set(unit, R_L2_AGE_TIMER, val);

    board_uport_to_lport(uport, &unit, &lport);
    val = (lport & 0x3F);
    val |= (0x1 << 28);
    bcm5333x_reg_set(unit, R_PER_PORT_AGE_CONTROL, val);

    for (i = 0; i < L2_AGE_TIMEOUT_COUNT; i++) {
        sal_usleep(100000);
        bcm5333x_reg_get(unit, R_PER_PORT_AGE_CONTROL, &val);
        if (val & (0x1 << 29)) {
            break;
        }
    }
...
board_l2_addr_get_first(board_l2_addr_t *l2addr, uint16 *index)
{
    ...
    *index = 0x0;
    ...
    for (i=0 ; i<L2_ENTRY_SIZE ; i++) {    
        rv |= bcm5333x_mem_get(0, M_L2_ENTRY(i), entry, 4);
        if (entry[3] & 0x8) {
            l2addr->mac[5] = (entry[0] >> 14) & 0xFF;
            l2addr->mac[4] = (entry[0] >> 22) & 0xFF;
            l2addr->mac[3] = ((entry[1] & 0x3F) << 2) | ((entry[0] >> 30) & 0x3);
            l2addr->mac[2] = (entry[1] >> 6) & 0xFF;
            l2addr->mac[1] = (entry[1] >> 14) & 0xFF;
            l2addr->mac[0] = (entry[1] >> 22) & 0xFF;
            l2addr->vid = (entry[0] >> 2) & 0xFFF;
            lport = ((entry[2] & 0xF) << 2) | ((entry[1] >> 30) & 0x3);
            if (entry[2] & 0x800) {
            ...
            if (l2addr->mac[0] & 0x1) {
                /* multicast */
                l2addr->mcidx = ((entry[2] & 0xFF) << 2) | ((entry[1] >> 30) & 0x3);
            } else {
                /* unicast */
                if (l2addr->is_trunk) {
                    l2addr->tgid = lport + 1;
