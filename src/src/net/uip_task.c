...
#define HTTPD_MSS               CFG_UIP_HTTPD_MSS
/*
*   New uip_task_send() to deal with the IP fragmenation with max. of 3 packets.
*/
static sys_error_t
uip_task_send(void)
{
    ...
    if (uip_len > MAX_PACKET_LENGTH) {
        remain_len = uip_len - MAX_PACKET_LENGTH;
        ...
        if (remain_len > HTTPD_MSS) {
            remain_len = remain_len - HTTPD_MSS;
            ...
            sal_memcpy((void *)&uip_buf[UIP_LLH_LEN + UIP_IPH_LEN],
                   (void *)&uip_buf[MAX_PACKET_LENGTH], (remain_len+HTTPD_MSS) );
                   
            pkt->pkt_data = (uint8 *)sal_dma_malloc(UIP_LLH_LEN + UIP_IPH_LEN + HTTPD_MSS + CRC_PADDING);
            if (pkt->pkt_data == NULL) {
                UIP_TASK_LOG(("Out of memory!\n"));
                sal_free(pkt);
                return SYS_ERR_OUT_OF_RESOURCE;
            ...
            UDPBUF->len[0] = ((UIP_IPH_LEN + HTTPD_MSS) >> 8);
            UDPBUF->len[1] = ((UIP_IPH_LEN + HTTPD_MSS) & 0xff);
    
            /* More fragment, Fragment offset = 1480/8 */
            UDPBUF->ipoffset[0] = 0x20;
            ...
            sal_memcpy(pkt->pkt_data, BUF, (UIP_LLH_LEN + UIP_IPH_LEN + HTTPD_MSS) );
            pkt->pkt_len = UIP_LLH_LEN + UIP_IPH_LEN + HTTPD_MSS + CRC_PADDING;
            ...
            /* Copy remaining data to begining of buffer */
            sal_memcpy((void *)&uip_buf[UIP_LLH_LEN + UIP_IPH_LEN],
                   (void *)&uip_buf[(UIP_LLH_LEN + UIP_IPH_LEN + HTTPD_MSS)], remain_len );
            
            pkt->pkt_data = (uint8 *)sal_dma_malloc(UIP_LLH_LEN + UIP_IPH_LEN + remain_len + CRC_PADDING);
            if (pkt->pkt_data == NULL) {
                UIP_TASK_LOG(("Out of memory!\n"));
                sal_free(pkt);
                return SYS_ERR_OUT_OF_RESOURCE;
            ...
            UDPBUF->len[0] = ((UIP_IPH_LEN + remain_len) >> 8);
            UDPBUF->len[1] = ((UIP_IPH_LEN + remain_len) & 0xff);
    
            /* Fragment offset = (1480+HTTPD_MSS)/8   */
            UDPBUF->ipoffset[0] = (((1480+HTTPD_MSS)/8) >> 8);
            UDPBUF->ipoffset[1] = (((1480+HTTPD_MSS)/8) & 0xFF);
            
            UDPBUF->ipchksum = 0;
            UDPBUF->ipchksum = uip_ipchksum();
    
            sal_memcpy(pkt->pkt_data, BUF, (UIP_LLH_LEN + UIP_IPH_LEN + remain_len) );
            pkt->pkt_len = UIP_LLH_LEN + UIP_IPH_LEN + remain_len + CRC_PADDING;
            ...
            /* Second packet of total 2 packets: (remain_len <= HTTPD_MSS) */
            /* Second packet */
            pkt = (sys_pkt_t *)sal_malloc(sizeof(sys_pkt_t));
            if (pkt == NULL) {
                UIP_TASK_LOG(("Out of memory!\n"));
...
#endif//if CFG_IP_FRAGMENT_INCLUDED
/*
