/*
 * $Id: flloop.c,v 1.10 Broadcom SDK $
 *
...
static uint32 loop_timer[BCM5607X_PORT_MAX+1];
static uint32 loop_counter[BCM5607X_PORT_MAX+1][3];
static BOOL loop_status[BCM5607X_PORT_MAX+1];
 ...
 */
STATICFN void
bcm5607x_loopdetect_task(void *param)
{
    uint8 i, link;
    uint32 counter_idx;
    uint16 uport;
    ...
    if (discovery_indicator & 0x1) {
        ...
        for (i = BCM5607X_LPORT_MIN; i <= BCM5607X_LPORT_MAX ; i++) {
            bcm5607x_link_status(0, i, &link);

            READ_FP_POLICY_TABLEm(0,LOOP_COUNT_IDX - BCM5607X_LPORT_MIN + (i), fp_policy_table);                          
...
STATICFN void
_bcm5607x_loop_detect_fp_init(void)
{
    ...
    for (lport = BCM5607X_LPORT_MIN; lport <= BCM5607X_LPORT_MAX; lport++) {
        ...
        WRITE_FP_TCAMm(0, LOOP_COUNT_IDX + (lport-BCM5607X_LPORT_MIN), fp_tcam);
        WRITE_FP_GLOBAL_MASK_TCAMm(0, LOOP_COUNT_IDX + (lport-BCM5607X_LPORT_MIN) , fp_global_mask_tcam);
	}
    ...
    for (lport = BCM5607X_LPORT_MIN; lport <= BCM5607X_LPORT_MAX; lport++) {
		...
		WRITE_FP_POLICY_TABLEm(0, LOOP_COUNT_IDX + (lport-BCM5607X_LPORT_MIN), fp_policy_table);
    }

    /* starting from CPU port */
    for (lport = 0; lport <= BCM5607X_LPORT_MAX; lport++) {
        if ((lport > 0) && (lport < BCM5607X_LPORT_MIN)) {
            continue;
        }
        ...
        if (lport == 0)
            ...
            WRITE_FP_TCAMm(0,LOOP_REDIRECT_IDX + (lport-BCM5607X_LPORT_MIN), fp_tcam);
        ...
        if (lport == 0)
            ...
            WRITE_FP_GLOBAL_MASK_TCAMm(0,LOOP_REDIRECT_IDX + (lport-BCM5607X_LPORT_MIN),fp_global_mask_tcam);
        
        PBMP_ASSIGN(all_mask, BCM5607X_ALL_PORTS_MASK);
        ...
        if (lport == 0)
            ...
            WRITE_FP_POLICY_TABLEm(0, LOOP_REDIRECT_IDX + (lport-BCM5607X_LPORT_MIN), fp_policy_table);
    }
...
STATICFN void
_bcm5607x_loop_detect_init(uint16 port)
{
...
#ifdef BSL_LS_SOC_LOOPDETECT_DEBUG
{
    ...
    if (spkt == NULL) {
        sal_printf("_bcm5607x_loop_detect_init: malloc failed!\n");
        return;
    }
    ...
    for (i = BCM5607X_LPORT_MIN; i <= BCM5607X_LPORT_MAX; i++) {
        ...
        READ_FP_POLICY_TABLEm(0,LOOP_COUNT_IDX + (i - BCM5607X_LPORT_MIN), fp_policy_table);
        counter_idx = FP_POLICY_TABLEm_COUNTER_INDEXf_GET(fp_policy_table);
    ...
    timer_add(bcm5607x_loopdetect_task, NULL, LOOP_DETECT_INTERVAL);
}

void
bcm5607x_loop_detect_init(void)
{
    LOG_VERBOSE(BSL_LS_SOC_COMMON,("%s..:\n", __func__));
...
#endif 
    _bcm5607x_loop_detect_init(0xA);
}

void
bcm5607x_loop_detect_enable(BOOL enable)
{
	...
	if (!enable) {
        timer_remove(bcm5607x_loopdetect_task);
        if (spkt) {
            sal_free(spkt);
            spkt = (soc_tx_packet_t *)NULL;
        }
    } else {
        bcm5607x_loop_detect_init();
    }
    loopdetect_status = enable;
}

uint8
bcm5607x_loop_detect_status_get(void)
{
