/*
 * $Id: flled.c,v 1.16 Broadcom SDK $
 *
...
const const unsigned char fl_generic[] = {
 ...
 ,
...
void bcm5607x_load_firmware(uint8 unit) {
#if 0 
   ...
   for (i = 0; i < LED_RAM_SIZE ; i++) {
        ...
        CMIC_LEDUP0_PROGRAM_RAMr_DATAf_SET(cmic_ledup0_program_ram, (i >= sizeof(fl_generic)) ? 0 : fl_generic[i]);
        ...
        CMIC_LEDUP1_PROGRAM_RAMr_DATAf_SET(cmic_ledup1_program_ram, (i >= sizeof(fl_generic)) ? 0 : fl_generic[i]);
...
void bcm5607x_load_customer_firmware(uint8 unit) {
#if 0 
...
/*
*   The define below should comply with fl_generic.asm
*/
...
#if 0 
#define LEDUP_AUTO_REMAP(u, v, x, y, z) bcm5607x_ledup_generic_auto_remap(u,v,x,y,z)

sys_error_t
bcm5607x_pport_to_ledup_port(uint8 unit, int pport, uint8 *ledup_no, uint8 *ledup_port) 
{
     int i, j;

    for (i=0; i< BCM5607X_LEDUP_NUMBER; i++) {
...
void bcm5607x_ledup_init(const ledup_ctrl_t *p_ledup_ctrl) {
#if 0 
    ...
    if (p_ledup_ctrl != NULL) {
        ...
        for (j=0; j < BCM5607X_LEDUP_NUMBER; j++) {
    ...
    bcm5607x_load_ledup_program(0); 
#endif
}

#if 0 
uint8 
bcm5607x_ledup_generic_auto_remap(uint8 unit, uint8 ledup_idx, u8a_t *ledup_remap, 
                                    ledup_auto_remap_entry_t *remap_entry, int remap_entry_num) 
{
        ...
        for (i = (remap_entry[j].core_number - 1); i >= 0; i--) {        
             if (PBMP_MEMBER(remap_entry[j].core_disable,i)) {
                      ...
                      pport = pport + pport_inc) 
                 {
                      ...
                      if ((bcm5607x_pport_to_ledup_port(unit, pport, &customer_ledup_no, &customer_ledup_port) == SYS_OK) &&
   ...
   SOC_LPORT_ITER(lport) {
        for (i=0; i<int_ledup_ctrl.leds_per_port; i++) {
             bcm5607x_ledup_mode_set(unit, lport, i, int_ledup_ctrl.led_mode[i]);
        }
   }   
...
void
bcm5607x_load_ledup_program(uint8 unit) {
#if 0 
   ...
   if (led_option == 3) {    
       bcm5607x_load_customer_firmware(unit);      
   }
...
*/
void 
bcm5607x_ledup_mode_set(uint8 unit, int lport, int led_no, int mode) {
#if 0 
   ...
   if (bcm5607x_pport_to_ledup_port(unit, SOC_PORT_L2P_MAPPING(lport), &ledup_no, &ledup_port) != SYS_OK) 
   {
       return; 
   }
...
void 
bcm5607x_ledup_color_set(uint8 unit, int lport, int color) {
#if 0 
    uint8  ledup_no, ledup_port;
    CMIC_LEDUP0_DATA_RAMr_t cmic_ledup0_data_ram;
    CMIC_LEDUP1_DATA_RAMr_t cmic_ledup1_data_ram;   
...
void 
bcm5607x_ledup_sw_linkup(uint8 unit, int lport, int linkup) {
#if 0 
