...
static void
_bcm5357x_lag_group_fp_set(uint8 unit, int start_index, uint8 lagid,
                     pbmp_t pbmp, pbmp_t old_pbmp, uint8 revise_redirect_pbmp, uint8 cpu_include)
{
    ...
    if (cpu_include == TRUE) {
        ...
        j = BCM5357X_LPORT_MIN;
    }
    for (i = start_index; j <= BCM5357X_LPORT_MAX; i++, j++) {
        if ((j > 0) && (j < BCM5357X_LPORT_MIN)) {
            continue;
        }
...
/*
 *  Function : bcm5357x_lag_group_set
 *  Purpose :
 ...
 */
sys_error_t
bcm5357x_lag_group_set(uint8 unit, uint8 lagid, pbmp_t lpbmp)
{
    uint8 i, j, count = 0;
    ...
    for (i = BCM5357X_LPORT_MIN; i <= BCM5357X_LPORT_MAX && count < BOARD_MAX_PORT_PER_LAG; i++) {
    ...
    if (PBMP_NEQ(lpbmp, old_lpbmp)) {
         ...
         * Rate ingress
         */
        _bcm5357x_lag_group_fp_set(unit, RATE_IGR_IDX, lagid,
         ...
         * Port based QoS
         */
        _bcm5357x_lag_group_fp_set(unit, QOS_BASE_IDX, lagid,
                                   lpbmp, old_lpbmp, FALSE, FALSE);
#endif /* CFG_SWITCH_QOS_INCLUDED */

#ifdef CFG_SWITCH_LOOPDETECT_INCLUDED
        /*
         * Slice 3 Entry BCM5357X_PORT_MIN~BCM5357X_PORT_MAX (one entry for each port):
         * Loop detect counter
         */
        _bcm5357x_lag_group_fp_set(unit, LOOP_COUNT_IDX, lagid,
                                   lpbmp, old_lpbmp, FALSE, FALSE);
         ...
         */
        _bcm5357x_lag_group_fp_set(unit, LOOP_REDIRECT_IDX, lagid,
                                   lpbmp, old_lpbmp, FALSE, FALSE);
                                   
        _bcm5357x_lag_group_fp_set(unit, LOOP_REDIRECT_CPU_IDX, lagid,
...
/*
 *  Function : bcm5357x_lag_group_get
 *  Purpose :
 ...
 */
void
bcm5357x_lag_group_get(uint8 unit, uint8 lagid, pbmp_t *pbmp) {
