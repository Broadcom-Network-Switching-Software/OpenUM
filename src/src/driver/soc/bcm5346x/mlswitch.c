...
uint8 viper_interface[VIPER_NUM_OF_CORES][4];
...

{
    ...
    if (op_code == SC_OP_L2_LOOKUP_CMD) {
        /* Check result */
        msg_hdr[0] = READCSR(R_CMIC_SCHAN_D(0));
        msg_hdr[1] = READCSR(R_CMIC_SCHAN_D(1));

        if (msg_hdr[0] & 0x1) {
            return SYS_ERR_NOT_FOUND;
        } else {
            *index = (uint16) msg_hdr[1];
        }
    }
...
bcm5346x_sw_init(void)
{
    ...
    for (core_num = 0; core_num < TSCE_NUM_OF_CORES; core_num++) {
         for (lane_num = 0; lane_num < 4; lane_num++) {
              tsce_interface[core_num][lane_num] = CFG_TSCE_INTERFACE;
              sal_config_rv = sal_config_uint8_xy_get(SAL_CONFIG_TSCE_INTERFACE, core_num, lane_num, &tsce_interface[core_num][lane_num]);
              if (sal_config_rv == SYS_OK) {
                  sal_printf("Vendor Config : Set TSCE interface (core %d lane %d) to %d. (1:SGMII, 2:Fiber)\n", core_num, lane_num, tsce_interface[core_num][lane_num]);
                  if ((tsce_interface[core_num][lane_num] != SERDES_INTERFACE_SGMII) && 
                      (tsce_interface[core_num][lane_num] != SERDES_INTERFACE_FIBER)) {
    ...
    for (core_num = 0; core_num < VIPER_NUM_OF_CORES; core_num++) {
         for (lane_num = 0; lane_num < 4; lane_num++) {
              viper_interface[core_num][lane_num] = CFG_VIPER_INTERFACE;
              sal_config_rv = sal_config_uint8_xy_get(SAL_CONFIG_VIPER_INTERFACE, core_num, lane_num, &viper_interface[core_num][lane_num]);
              if (sal_config_rv == SYS_OK) {
                  sal_printf("Vendor Config : Set VIPER interface (core %d lane %d) to %d. (1:SGMII, 2:Fiber)\n", core_num, lane_num, viper_interface[core_num][lane_num]);
                  if ((viper_interface[core_num][lane_num] != SERDES_INTERFACE_SGMII) && 
                      (viper_interface[core_num][lane_num] != SERDES_INTERFACE_FIBER)) {
    ...
    SOC_LPORT_ITER(lport) {
        ...
        if ((port_speed[lport] == 1000) && 
            ...
            if (IS_XL_PORT(lport)) {
                tsce_if = tsce_interface[TSCE_CORE_NUM_GET(lport)][SOC_PORT_BLOCK_INDEX(lport)];
                if (tsce_if == SERDES_INTERFACE_SGMII) {
                    rv = bmd_phy_mode_set(unit, lport, "bcmi_tsce_xgxs",
                ...
                viper_if = viper_interface[VIPER_CORE_NUM_GET(lport)][SOC_PORT_BLOCK_INDEX(lport)];
                if (viper_if == SERDES_INTERFACE_SGMII) {
                    rv = bmd_phy_mode_set(unit, lport, "bcmi_viper_xgxs",
                              BMD_PHY_MODE_FIBER, 0);
                } else {
                    rv = bmd_phy_mode_set(unit, lport, "bcmi_viper_xgxs",
        ...
        if (port_speed[lport] == 1000) {
            ...
            if ((IS_XL_PORT(lport) && (tsce_if == SERDES_INTERFACE_SGMII)) ||
                (IS_GX_PORT(lport) && (viper_if == SERDES_INTERFACE_SGMII))) {
                an = 0;
            } else {
