...
int soc_hurricane3_pgw_rx_fifo_reset(int unit, int lport, int reset)
{
    int phy_port = SOC_PORT_L2P_MAPPING(lport);
    int block_id = -1;
    int pindex = -1;
    PGW_GE_RXFIFO_SOFT_RESETr_t pgw_ge_rxfifo_soft_reset;
    

    if (SOC_PORT_BLOCK_TYPE(phy_port) != PORT_BLOCK_TYPE_GXPORT) {
        return SYS_ERR;
    }
    
    block_id = bcm5354x_pgw_ge_pport_to_blockid[phy_port];
    pindex = bcm5354x_pgw_ge_pport_to_index_in_block[phy_port];
    if (block_id < 0) {
        return SYS_ERR;
    }
    
    READ_PGW_GE_RXFIFO_SOFT_RESETr(unit, lport, pgw_ge_rxfifo_soft_reset);
    if (reset) {
        pgw_ge_rxfifo_soft_reset.v[0] |= 1 << pindex;
    } else {
        pgw_ge_rxfifo_soft_reset.v[0] &= ~(1 << pindex);
    }
    WRITE_PGW_GE_RXFIFO_SOFT_RESETr(unit, lport, pgw_ge_rxfifo_soft_reset);
    
    return SYS_OK;
}


int soc_pgw_rx_fifo_reset(int unit, int lport, int reset)
{
    soc_hurricane3_pgw_rx_fifo_reset(unit, lport, reset);
    return SYS_OK;
}
...
soc_egress_drain_cells(uint8 unit, uint8 lport, uint32 drain_timeout)
{
    ...
    READ_EGRMETERINGCONFIGr(unit, lport,  orig_egrmeteringconfig);
    EGRMETERINGCONFIGr_CLR(egrmeteringconfig);
    WRITE_EGRMETERINGCONFIGr(unit, lport, egrmeteringconfig);
    ...
    do {
        ...
        for (cos = 0; cos < COS_QUEUE_NUM; cos++) {           
            READ_COSLCCOUNTr(unit, lport, cos, coslccount);
            count += COSLCCOUNTr_LCCOUNTf_GET(coslccount);
        }
    } while (count != 0);

    WRITE_EGRMETERINGCONFIGr(unit, lport, orig_egrmeteringconfig);
...
_mac_uni_sw_reset(uint8 unit, uint8 lport, BOOL reset_assert)
{
    ...
    if (reset_assert) {
        soc_pgw_rx_fifo_reset(unit, lport, TRUE);
    }
    ...
    if (!reset_assert) {
        soc_pgw_rx_fifo_reset(unit, lport, FALSE);
    }
...
mac_uni_pause_get(int unit, uint8 lport, int *pause_tx, int *pause_rx)
{
    ...
    *pause_rx = COMMAND_CONFIGr_PAUSE_IGNOREf_GET(command_config) == 0;
    *pause_tx =  COMMAND_CONFIGr_IGNORE_TX_PAUSEf_GET(command_config) == 0;


    LOCAL_PRINTF("mac_uni_pause_get: port %d RX=%s TX=%s\n", lport,
                 *pause_rx ? "on" : "off",
                 *pause_tx ? "on" : "off");
    return SYS_OK;
}
...
mac_uni_pause_set(int unit, uint8 lport, int pause_tx, int pause_rx)
{
	...
	LOCAL_PRINTF("mac_uni_pause_set: port %d RX=%s TX=%s\n", lport,
				  pause_rx ? "on" : "off",
				  pause_tx ? "on" : "off");
...
_mac_uni_drain_cells(uint8 unit, uint8 lport)
{
    ...
    PBMP_PORT_ADD(*((pbmp_t *) &mmuflushcontrol), lport);
...
mac_uni_enable_set(int unit, uint8 lport, int enable)
{
	...
	LOCAL_PRINTF("mac_uni_enable_set: unit %d port %d enable=%s\n", unit, lport, enable ? "True" : "False");
...
mac_uni_init(int unit, uint8 lport)
{
    ...
    MAC_0r_MAC_ADDR0f_SET(mac_0, (system_mac[0] << 24 | system_mac[1] << 16 | system_mac[2] << 8 | system_mac[3] << 0));
    WRITE_MAC_0r(unit, lport, mac_0);
    MAC_1r_MAC_ADDR1f_SET(mac_1, (system_mac[4] << 8 | system_mac[5]));
    WRITE_MAC_1r(unit, lport, mac_1);
...
static int mac_uni_speed_get(int unit, uint8 lport, int *speed) 
{
	    ...
	    LOCAL_PRINTF("mac_uni_speed_get: unit %d port %d speed=%dMb\n", unit, lport, *speed);
...
mac_uni_duplex_get(int unit, uint8 lport, int *duplex)
{
    ...
    LOCAL_PRINTF("mac_uni_duplex_get: unit %d port %d duplex=%s\n",unit, lport,*duplex ? "Full" : "Half");
    return SYS_OK;
}
...
mac_uni_duplex_set(int unit, uint8 lport, int duplex)
{
    ...
    LOCAL_PRINTF("mac_uni_duplex_set: unit %d port %d duplex=%s\n",unit, lport, duplex ? "Full" : "Half");
...
mac_uni_speed_set(int unit, uint8 lport, int speed)
{
	...
	LOCAL_PRINTF("mac_uni_speed_set: unit %d port %d speed=%d\n",unit, lport, speed);
...
mac_uni_enable_get(int unit, uint8 lport, int *enable)
{
    ...
    *enable = COMMAND_CONFIGr_RX_ENAf_GET(command_config);
    ...
    LOCAL_PRINTF("mac_uni_enable_get: unit %d port %d enable=%s\n", unit, lport, *enable ? "True" : "False");
...
mac_uni_loopback_set(int unit, uint8 lport, int lb)
{
	...
	LOCAL_PRINTF("mac_uni_loopback_set %d %d\n", lport, lb);
...
mac_uni_loopback_get(int unit, uint8 lport, int *lb)
{
    ...
    *lb = COMMAND_CONFIGr_LOOP_ENAf_GET(command_config);
...
mac_uni_ability_local_get(int unit, uint8 lport, soc_port_ability_t *ability) {
   ...
   LOCAL_PRINTF("mac_uni_ability_local_get %d\n", lport);
...
mac_uni_interface_get(int unit, uint8 lport, soc_port_if_t *pif)
{
    *pif = SOC_PORT_IF_GMII;
    ...
    LOCAL_PRINTF("mac_uni_interface_get: unit %d port %d interface=%d\n", unit, lport, *pif);
                    ...
                    int value)
{
    ...
    LOCAL_PRINTF("mac_uni_control_set: unit %d port %d type=%d value=%d\n", unit, lport, type, value);
                    ...
                    int *value)
{
   ...
   LOCAL_PRINTF("mac_uni_control_get: unit %d port %d type=%d value=%d rv=%d\n", unit, lport, type, *value, rv);
   return rv;
}
...
mac_uni_pause_addr_set(int unit, uint8 lport, sal_mac_addr_t pause_mac)
{
	...
	LOCAL_PRINTF("mac_uni_pause_addr_set: unit %d port %d "
...
mac_uni_pause_addr_get(int unit, uint8 lport, sal_mac_addr_t pause_mac)
{
	...
	LOCAL_PRINTF("mac_uni_pause_addr_get: unit %d port %d "
                             "MAC=<%02x:%02x:%02x:%02x:%02x:%02x>\n",
                 unit, lport,
                 pause_mac[0], pause_mac[1], pause_mac[2],
                 pause_mac[3], pause_mac[4], pause_mac[5]);
                ...
                soc_port_duplex_t  duplex, int ifg)
{
    ...
    LOCAL_PRINTF("mac_uni_ifg_set: unit %d port %d speed=%dMb duplex=%s "
                             "ifg=%d\n",
                 unit, lport,
...
mac_uni_frame_max_set(int unit, uint8 lport, int size)
{
    ...
    LOCAL_PRINTF("mac_uni_frame_max_set: unit %d port %d size=%d\n", unit, lport, size);
...
mac_uni_frame_max_get(int unit, uint8 lport, int *size)
{
    ...
    *size = (int) FRM_LENGTHr_MAXFRf_GET(frame_length);
	...
	LOCAL_PRINTF("mac_uni_frame_max_get: unit %d port %s size=%d\n", unit, lport,*size);
    return SYS_OK;
}
/*
 * Function:
...
mac_uni_ability_get(int unit, uint8 lport, soc_port_mode_t *mode)
{
   *mode = (SOC_PM_10MB | SOC_PM_100MB | SOC_PM_1000MB_FD |
    ...
    LOCAL_PRINTF("mac_uni_ability_get: unit %d port %d mode=0x%x\n",
                 unit, lport, *mode);
    return SYS_OK;
}
