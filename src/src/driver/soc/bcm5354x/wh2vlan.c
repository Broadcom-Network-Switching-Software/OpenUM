...
static vlan_type_t wh2_vlan_type = VT_COUNT;
/*
 *  Function : bcm5354x_pvlan_egress_set
 *
 *  Purpose :
...
sys_error_t 
bcm5354x_pvlan_egress_set(uint8 unit, uint8 lport, pbmp_t lpbmp)
{
    ...
    IFP_REDIRECTION_PROFILEm_BITMAPf_SET(ifp_redirection_profile, SOC_PBMP(lpbmp));
...
/*
 *  Function : bcm5354x_pvlan_egress_get
 *
 *  Purpose :
 ...
 */
sys_error_t
bcm5354x_pvlan_egress_get(uint8 unit, uint8 lport, pbmp_t *lpbmp)
{
    sys_error_t rv = SYS_OK;
...
sys_error_t 
bcm5354x_qvlan_port_set(uint8 unit, uint16  vlan_id, pbmp_t lpbmp, pbmp_t tag_lpbmp)
{
    sys_error_t rv = SYS_OK;
 ...
 sys_error_t
bcm5354x_qvlan_port_get(uint8 unit, uint16  vlan_id, pbmp_t *lpbmp, pbmp_t *tag_lpbmp)
{
    sys_error_t rv = SYS_OK;
    ...
    PBMP_WORD_SET(untag_lpbmp, 0, EGR_VLANm_UT_BITMAPf_GET(egr_vlan));
    PBMP_CLEAR(*lpbmp);
    PBMP_WORD_SET((*lpbmp), 0, EGR_VLANm_PORT_BITMAPf_GET(egr_vlan));
 ...
 */
sys_error_t
bcm5354x_vlan_create(uint8 unit, vlan_type_t type, uint16  vlan_id)
{
    sys_error_t rv = SYS_OK;
...
/*
 *  Function : _bcm5354x_vlan_destroy
 *
 *  Purpose :
 *      Destroy vlan.
 *
 ...
 */
sys_error_t
bcm5354x_vlan_destroy(uint8 unit, uint16  vlan_id)
{
...
/*
 *  Function : bcm5354x_vlan_type_set
 *
 *  Purpose :
 ...
 */
sys_error_t
bcm5354x_vlan_type_set(uint8 unit, vlan_type_t type)
{
    int i;
    ...
    if ((type == VT_NONE) || (type == VT_PORT_BASED)) 
        ...
        if ((wh2_vlan_type == VT_NONE) || (wh2_vlan_type == VT_PORT_BASED)) { 
             wh2_vlan_type = type;
             return SYS_OK;
        }
        ...
        VLANm_CLR(vlan);
        VLANm_PORT_BITMAPf_SET(vlan, SOC_PBMP(BCM5354X_ALL_PORTS_MASK));
        VLANm_STGf_SET(vlan, 1);
        VLANm_VALIDf_SET(vlan, 1);
        VLANm_VLAN_PROFILE_PTRf_SET(vlan, 0);
        /* create 2-4094 vlans */
        ...
        EGR_VLANm_CLR(egr_vlan);
        EGR_VLANm_PORT_BITMAPf_SET(egr_vlan, SOC_PBMP(BCM5354X_ALL_PORTS_MASK));
        EGR_VLANm_STGf_SET(egr_vlan, 1);
        EGR_VLANm_VALIDf_SET(egr_vlan, 1);
        for (i = 2; i <= 4094; i++) {
 ...
 */
sys_error_t
bcm5354x_vlan_reset(uint8 unit)
{
    sys_error_t rv = SYS_OK;
    int lport;
    ...
    EGR_VLANm_CLR(egr_vlan);
    EGR_VLANm_UT_BITMAPf_SET(egr_vlan, SOC_PBMP(BCM5354X_ALL_PORTS_MASK));
    EGR_VLANm_PORT_BITMAPf_SET(egr_vlan, SOC_PBMP(BCM5354X_ALL_PORTS_MASK));
    EGR_VLANm_STGf_SET(egr_vlan, 1);
    EGR_VLANm_VALIDf_SET(egr_vlan, 1);
