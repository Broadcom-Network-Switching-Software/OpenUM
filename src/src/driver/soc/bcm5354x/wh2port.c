...
static uint8 eee_state[BCM5354X_LPORT_MAX+1];
...
bcm5354x_rate_init(void)
{
    ...
    for (lport = BCM5354X_LPORT_MIN; lport <= BCM5354X_LPORT_MAX; lport++) {
        ...
        FP_PORT_FIELD_SELm_SLICE1_F3f_SET(fp_port_field_sel, 11);
        ...
        WRITE_FP_TCAMm(0, RATE_IGR_IDX + lport - BCM5354X_LPORT_MIN , fp_tcam);

        FP_GLOBAL_MASK_TCAMm_CLR(fp_global_mask_tcam);
        FP_GLOBAL_MASK_TCAMm_VALIDf_SET(fp_global_mask_tcam, 1);
        WRITE_FP_GLOBAL_MASK_TCAMm(0, RATE_IGR_IDX + lport - BCM5354X_LPORT_MIN, fp_global_mask_tcam);
        ...
        WRITE_FP_POLICY_TABLEm(0, RATE_IGR_IDX + (lport - BCM5354X_LPORT_MIN), fp_policy_table);          
	}
...
bcm5354x_qos_init(void)
{
    ...
    for (i = BCM5354X_LPORT_MIN; i <= BCM5354X_LPORT_MAX; i++) {
        ...
        FP_GLOBAL_MASK_TCAMm_CLR(fp_global_mask_tcam);
        FP_GLOBAL_MASK_TCAMm_VALIDf_SET(fp_global_mask_tcam, 1);
        WRITE_FP_GLOBAL_MASK_TCAMm(0, QOS_BASE_IDX + (i - BCM5354X_LPORT_MIN), fp_global_mask_tcam);
    ...
    for (j = BCM5354X_LPORT_MIN; j <= BCM5354X_LPORT_MAX; j++) {
...
void
bcm5354x_dscp_map_enable(BOOL enable)
{
    int i;
    PORTm_t port;
...
/*
 *  Function : bcm5354x_port_eee_enable_set
 *
 *  Purpose :
 ...
 */
sys_error_t
bcm5354x_port_eee_enable_set(uint8 unit, uint8 lport, uint8 enable, uint8 save)
{
...
/*
 *  Function : bcm5354x_port_eee_enable_get
 *
 *  Purpose :
 ...
 */
void
bcm5354x_port_eee_enable_get(uint8 unit, uint8 lport, uint8 *enable)
{
