...
int
pcm_phyctrl_wolfhound2_port_probe_init(int unit, pbmp_t all_lpbmp, pbmp_t *okay_lpbmp) {
    ...
    rv = pcm_phyctrl_port_probe_init(unit, BCM5354X_ALL_PORTS_MASK, okay_lpbmp);
    ...
    SOC_LPORT_ITER(lport) {
        ...
        if (sal_config_pbmp_get(SAL_CONFIG_PHY_AN_PORTS, &lpbmp) == SYS_OK) {
            sal_printf("Vendor Config : Set AN logical pbmp with value %s.\n", SOC_PBMP_FORMAT(lpbmp));
			SOC_LPORT_ITER(lport) {
				if (PBMP_MEMBER(lpbmp, lport)) {
				    sal_printf("lport=%d pcm_port_autoneg_set 1\n", lport);
					pcm_port_autoneg_set(unit, lport, 1);
					
				} else {
				    //sal_printf("lport=%d pcm_port_autoneg_set 0\n", lport);
...
int
bcm5354x_pcm_software_init(int unit) {
    ...
    if (sal_config_uint8_get("serdes_fiber_pref", &val8) != SYS_OK) {
        ...
        for (core_num = 0; core_num < SGMIIP4_NUM_OF_CORES; core_num++) {
            if(core_num ==0) {
                for (lane_num = 0; lane_num < 4; lane_num++) {
                    if (sgmiipx4_interface[core_num][lane_num] == SGMIIPX4_INTERFACE_FIBER) {
                        if (SOC_PORT_P2L_MAPPING(PHY_GPORT3_BASE + lane_num ) >= 2) {
                            PBMP_PORT_ADD(lpbmp, SOC_PORT_P2L_MAPPING(PHY_GPORT3_BASE + lane_num ) );
                        } 
                    }
                }
            }
            if(core_num ==1) {
                for (lane_num = 0; lane_num < 4; lane_num++) {
                    if (sgmiipx4_interface[core_num][lane_num] == SGMIIPX4_INTERFACE_FIBER) {
                        if (SOC_PORT_P2L_MAPPING(PHY_GPORT4_BASE + lane_num ) >= 2) {
                            PBMP_PORT_ADD(lpbmp, SOC_PORT_P2L_MAPPING(PHY_GPORT4_BASE + lane_num ) );
                        } 
                    }
                }
            }
