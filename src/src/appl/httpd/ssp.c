...
#define _SSP_TRACEX(fn) sal_printf("ssp: %s, wh=%p file=%s\n", #fn, wh, \
        (wh == NULL || SSPH->pfile == NULL)? "" : SSPH->pfile->name);
#else 
#define _SSP_TRACE()
...
cmd_diagssp(int argc, char *argv[])
{
    ...
    sal_printf("  Last callback called: %p\n", ssp_diag_last_called);
    sal_printf("  Last parameters: (%u, %u, %u)\n",
    ...
    for(;;) {
        ...
        case SSP_DATA_TYPE_LOOP:
            ...
            SSP_DBG(("Loop tag %p (%u,%u,%u) for %s", 
                    ...
                    SSPH->pfile->name));
            _SSP_DIAG_CALL((void *)SSPH->page->dataptr);
            _SSP_DIAG_PARAMS();
            SSP_TRACING();
