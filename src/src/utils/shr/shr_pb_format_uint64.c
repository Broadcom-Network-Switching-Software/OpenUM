                     ...
                     const uint64_t_fixme *val, int count, uint32_t bitwidth)
{
    ...
    if (minwidth > 0) {
        ...
        for (idx = start; idx >= 0; idx--) {
            ...
            if (idx == start) {
                if (minwidth > 8) {
                    shr_pb_printf(pb, "0x%.*"SAL_PRIx64, minwidth - 8, val64 >> 32);
                    shr_pb_printf(pb, "_%08"SAL_PRIx64, val64 & 0xffffffff);
                } else {
                    shr_pb_printf(pb, "0x%.*"SAL_PRIx64, minwidth, val64);
                }
            } else {
                shr_pb_printf(pb, "_%08"SAL_PRIx64, val64 >> 32);
                shr_pb_printf(pb, "_%08"SAL_PRIx64, val64 & 0xffffffff);
            }
        }
    } else {
        ...
        for (start = -1, idx = count - 1; idx >= 0; idx--) {
            if (start < 0) {
                if (idx == 0 && val[idx] < 10) {
                    shr_pb_printf(pb, "%"SAL_PRIu64, val[idx]);
                ...
                if (val[idx] > 0xffffffff) {
                    shr_pb_printf(pb, "0x%"SAL_PRIx64, val[idx] >> 32);
                    shr_pb_printf(pb, "_%08"SAL_PRIx64, val[idx] & 0xffffffff);
                } else {
                    shr_pb_printf(pb, "0x%"SAL_PRIx64, val[idx]);
                }
                start = idx;
                continue;
            }
            shr_pb_printf(pb, "_%08"SAL_PRIx64, val[idx] >> 32);
            shr_pb_printf(pb, "_%08"SAL_PRIx64, val[idx] & 0xffffffff);
        }
    }
