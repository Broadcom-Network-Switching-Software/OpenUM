                     ...
                     const uint32_t *val, int count, uint32_t bitwidth)
{
    ...
    if (minwidth > 0) {
        ...
        for (idx = start; idx >= 0; idx--) {
            ...
            if (idx == start) {
                shr_pb_printf(pb, "0x%.*"SAL_PRIx32, minwidth, val32);
            } else {
                shr_pb_printf(pb, "_%08"SAL_PRIx32, val32);
            }
        }
    } else {
        /*
        ...
        for (start = -1, idx = count - 1; idx >= 0; idx--) {
            if (start < 0) {
                if (idx == 0 && val[idx] < 10) {
                    shr_pb_printf(pb, "%"SAL_PRIu32, *val);
                ...
                shr_pb_printf(pb, "0x%"SAL_PRIx32, val[idx]);
                start = idx;
                continue;
            }
            shr_pb_printf(pb, "_%08"SAL_PRIx32, val[idx]);
        }
    }
