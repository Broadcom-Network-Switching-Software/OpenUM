/*
 * $Id: nvram_utils.c,v 1.20 Broadcom SDK $
 *
...
#if !defined(__BOOTLOADER__) || defined(CFG_VENDOR_CONFIG_SUPPORT_INCLUDED)
static BOOL
...
nvram_commit()
{
    ...
    uint32 buf_size;    
    char chksum[5];
    /* Assume NVRAM_BASE is block alignment */
    block_size = flash_block_size(NVRAM_BASE);
    if (NVRAM_BASE & (block_size - 1)) {
#if CFG_CONSOLE_ENABLED
        sal_printf("nvram_commit: base address should align with FLASH block size!\n");
#endif
       return SYS_ERR;
    }

    buf_size = ((NVRAM_SPACE + NVRAM_OFFSET + block_size - 1)/(block_size)) * block_size;   

    /* Allocate and copy block data */
    buf = sal_malloc(buf_size);
    if (!buf) {
#if CFG_CONSOLE_ENABLED
    ...
    flash_read(NVRAM_BASE, buf, buf_size);
    ...
    res = flash_erase((hsaddr_t)NVRAM_BASE, buf_size);
    if (res != SYS_OK) {
#if CFG_CONSOLE_ENABLED
    ...
    res = flash_program((hsaddr_t)NVRAM_BASE, (const void *)buf, buf_size);
